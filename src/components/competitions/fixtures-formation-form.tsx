...
import { replaceFixture } from "@/stores/features/services/club/fixtures";

type Title = "goalKeepers" | "defenders" | "midFielders" | "forwards";
const TEAM_SIZE = 11;

export default function FixturesFormationForm({
  open,
  onClose,
  competitionId,
}: {
  ...
  const onChange = useCallback(
    (value: string, title: Title) => {
      const keepersCount = selectedList.goalKeepers.length;
      const defendersCount = selectedList.defenders.length;
      const midFieldersCount = selectedList.midFielders.length;
      const forwardsCount = selectedList.forwards.length;
      const totalSelectedPlayers =
        keepersCount + defendersCount + midFieldersCount + forwardsCount;
      if (totalSelectedPlayers >= TEAM_SIZE) {
        dispatch(
          openInfobar({
            message: "Maximum participant exceeded!",
            isError: true,
          })
        );
        return;
      }
      if (keepersCount > 0 && title === "goalKeepers") {
        dispatch(
          openInfobar({
            message: "Maximum goalkeeper exceeded!",
            isError: true,
          })
        );
        return;
  ...
  const submitForm = useCallback(() => {
    try {
      ...
      dispatch(
        openInfobar({
          message: "Fixture formation configured!",
          isError: false,
        })
      );
      dispatch(replaceFixture({ _id: competitionId, isFormationSet: true }));
      onClose(false);
    } catch (error) {
    } finally {
    ...
    >
      ...
      >
        ...
        <div className=" flex flex-col gap-1 dark:bg-dark-ash-700/60 bg-gray-50 rounded-lg">
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder="Goal Keeper" />
            </SelectTrigger>
