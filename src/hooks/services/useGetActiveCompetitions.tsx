...
} from "@/stores/features/services/active-competitions";
import competitions from "@/mock-up-data/competitions.json";
//axios instace interceptor for access token integration and refresh tokens
export default function useGetActiveCompetitions({
  page = 1,
  limit = 10,
  start_date,
  end_date,
  ...
  } = useAppSelector((state) => state.activeCompetition.value);
    ...
    async (skipCache?: boolean, limitless?: number) => {
      ...
      try {
        ...
        if (foundPage && !remakeRequest && !skipCache) {
          ...
          dispatch(updateActiveCompetition({ data: foundPage?.data }));
        } else {
          ...
          //   length: data?.length,
          // };
          const data = competitions?.active_competitions;
          const paginationDataset = {
            current_page: 1,
            last_page: 1,
            per_page: 10,
          ...
          dispatch(updateActiveCompetition({ data }));
          if (!remakeRequest) {
            dispatch(
              addToPaginationHistory({
