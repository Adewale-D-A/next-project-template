...
} from "@/stores/features/services/scout/watchlist";
...
export default function useGetWatchlist({
  ...
  } = useAppSelector((state) => state.watchlist.value);
    ...
    async (skipCache?: boolean, limitless?: number) => {
      ...
      try {
        ...
        if (foundPage && !remakeRequest && !skipCache) {
          ...
          dispatch(updateWatchlist({ data: foundPage?.data }));
        } else {
          // const response = await axios.get(`/my-watchlist?${queryString}`);
          ...
          //   length: data?.length,
          // };
          const data = players?.multiple_players?.slice(0, 4);
          const paginationDataset = {
            current_page: 1,
            last_page: 1,
            per_page: 10,
